...
namespace CLUtils {
	...
	const char* GetErrorName(cl_int errorCode) {
		switch (errorCode) {
			case CL_SUCCESS:
				return "CL_SUCCESS";
	...
	cl_program CompileProgramFromFiles(cl_context Context, cl_device_id Device, unsigned int SourceCount, const char** SourcePaths, const char* BuildFlags) {
		size_t* Kernel_Lengths = (size_t*) calloc(SourceCount, sizeof(size_t));
		char** Kernel_Sources = (char**) calloc(SourceCount, sizeof(char*));
		
		for (unsigned int SourceIndex = 0; SourceIndex < SourceCount; SourceIndex++) {
			Kernel_Sources[SourceIndex] = Utils::ReadFile(SourcePaths[SourceIndex], false, &Kernel_Lengths[SourceIndex]); //TODO free the string this allocates
		}
		
		cl_int Err;
		
		cl_program Program = clCreateProgramWithSource(Context, SourceCount, (const char**)Kernel_Sources, Kernel_Lengths, &Err);
		Err = clBuildProgram(Program, 1, &Device, BuildFlags, NULL, NULL);
